...
async def alert(ctx, *, msg): #COMMAND NAME
    for guild in client.guilds:
        ...
        for channel in guild.channels:
            if channel.name in ('wafaduckalertss', 'wafaduck-alerts', '👑│🐤wafaduck-alerts'):  # change for the list of channel names you got in your discord server
    ...
    for guild in client.guilds:
        ...
        for channel in guild.channels:
            if channel.name in ('wafaduckalertss', 'wafaduck-alerts', '👑│🐤wafaduck-alerts'):  # change for the channel names
                ...
                two = Button(style=ButtonStyle.URL, label='Discord', url="https://discord.gg/darfN8Kq")
                """three = Button(style=ButtonStyle.URL, label='Trading Journal', url="https://tradingjournal.com")"""
                embed=discord.Embed(title= ':money_with_wings: **Crypto Alert** :money_with_wings:', description= (msg), url='https://twitter.com/lowfloatmomo', color=0x33FF9F, timestamp=datetime.datetime.utcnow())
                """embed.set_author(name="Crypto Alert", icon_url = ctx.author.avatar_url)""" #Top left name # IGNORE
                embed.set_thumbnail(url='https://c1.peakpx.com/wallpaper/344/741/154/bull-sculpture-ox-horns-animal-wallpaper.jpg')
...
"""@client.command() #v1 #WITH BUTTONS // IGNORE
@commands.has_permissions(kick_members=True)
...
client.run('3H6jGMUQ7zq7DiT62FuLBllkKr7JZvoZ')#token
